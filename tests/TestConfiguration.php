...
class TestConfiguration {
  ...
  public static function getConfiguration() {
    ...
    if (!file_exists($localPath)) {
      throw new \Exception(sprintf('%s does not exist. Copy %s to %s and fill out the values.', $localPath, $examplePath, $localPath));
    }
    elseif (!is_readable($localPath)) {
      ...
      foreach ($requiredKeys as $requiredKey) {
        if (!isset($requiredKey, $configuration)) {
          throw new \Exception(sprintf('Configuration key %s is required.', $requiredKey));
        }
      }
